...
def compute_image_gradients(image_bw: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
    ...
    k_h = SOBEL_X_KERNEL.shape[0]
    k_w = SOBEL_X_KERNEL.shape[1]
    pad_h = k_h // 2
    pad_w = k_w // 2
    ...
    for i in range(m):
        for j in range(n):
            ...
            Ix[i, j] = np.sum(window * SOBEL_X_KERNEL)
            Iy[i, j] = np.sum(window * SOBEL_Y_KERNEL)
...
def compute_harris_response_map(
    ...
    sx2, sy2, sxsy = second_moments(image_bw, ksize=ksize, sigma=sigma)
...
def get_harris_interest_points(
    ...
    # non-maximum suppression and keep top k
    x, y, c = nms_maxpool_pytorch(R_norm, k=k, ksize=7)

    # remove border values unsuitable for 16x16 SIFT extraction
